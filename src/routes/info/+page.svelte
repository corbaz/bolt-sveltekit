...
<div class="flex flex-col h-[calc(100vh-64px)] mt-16">
  ...
  <div class="fixed top-16 left-0 right-0 bg-white z-10 py-4 shadow-sm">
    <div class="flex justify-center gap-4">
      <button
        class="px-4 py-2 rounded transition-all hover:scale-105 {$language === 'en' ? 'bg-blue-200 text-blue-800' : 'bg-gray-100 text-gray-600'}"
        on:click={() => language.set('en')}>
        English
      </button>
      <button
        class="px-4 py-2 rounded transition-all hover:scale-105 {$language === 'es' ? 'bg-blue-200 text-blue-800' : 'bg-gray-100 text-gray-600'}"
